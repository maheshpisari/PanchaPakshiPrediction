...
# Pancha Pakshi Activity Order:
# 1. Ruling (Arasu), 2. Eating (Oon), 3. Walking (Nadai), 4. Sleeping (Thuyil), 5. Dying (Saavu)
...
# Master Activity Sequence: [Ruling, Eating, Walking, Sleeping, Dying]
# Mapping day sequences per weekday for Shukla and Krishna Paksha
SHUKLA_DAY_ORDER = {
...
def get_panchang_and_yamas(date_obj, selected_bird, lat="12.9754", lon="77.6783"):
    """
    Computes exact Sunrise, Sunset, Paksha, Day Bird, and 5 daytime Yamas.
    ...
    # Search window relative to midnight IST
    ...
    # Compute Lunar Phase for Tithi & Paksha at local noon
    ...
    moon_lon = float(m_ecl.lon) * 180.0 / 3.141592653589793
    sun_lon = float(s_ecl.lon) * 180.0 / 3.141592653589793
    ...
    # Calculate 5 equal Yamas during day
    day_duration = sunset - sunrise
    yama_len = day_duration / 5

    # Determine activity order for selected bird
...
with c1:
    selected_bird = st.selectbox("Select Your Birth Bird", BIRDS, index=3) # Default: Cock
with c2:
...
def generate_cell_html(day_obj, row_idx, col_idx):
    ...
    # Dynamic tooltip positioning to prevent offscreen cutoffs
    ...
    <div class='tooltip-content' style='{pos_x} {pos_y}'>
        ...
        <div class='tip-body'>
            <div class='tip-row'><span>Paksha:</span> <strong>{data['paksha']} (Tithi {data['tithi_no']})</strong></div>
            <div class='tip-row'><span>Sunrise / Sunset:</span> <strong>{data['sunrise']} | {data['sunset']}</strong></div>
            ...
            <div style='font-size: 10px; color: #94a3b8;'>
                • <strong>Ruling/Eating:</strong> Highest probability entry windows.<br>
                • <strong>Sleeping/Dying:</strong> Stop-loss hunting / capital protection windows.
            </div>
        </div>
    </div>
    """
...
# Embedded Self-Contained Stylesheet
full_html = f"""
<!DOCTYPE html>
<html>
<head>
...
<style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }}
    ...
    /* Paksha Colors */
    .shukla-cell {{ background-color: #f1f5f9; }} /* Light Grey */
    .krishna-cell {{ background-color: #fef9c3; }} /* Light Yellow */
    ...
    .cal-cell:hover .tooltip-content {{
        display: block;
    }}
...
# Render full height so all 5/6 weeks display cleanly without internal scrolling
st.components.v1.html(full_html, height=1350, scrolling=True)

# --- DECISION-MAKING SUMMARY ---
st.markdown("""
---
### 📊 Trading Framework & Activity Legend
* 🟩 **Ruling (Arasu):** Highest conviction phase. Peak cosmic alignment for entering swing/intraday breakout positions.
* 🟩 **Eating (Bhojana):** Wealth expansion phase. Ideal for position sizing, trailing profits, or scaling in.
* ⬜ **Walking (Nadai):** Neutral transit phase. Good for routine management; avoid opening heavy aggressive bets.
* 🟥 **Sleeping (Thuyil):** Energy exhaustion. Trades frequently experience false breakouts or momentum stalls.
* 🟥 **Dying (Saavu):** Hostile phase. Highest frequency of stop-loss hunts. Strictly preserve capital.
""")
